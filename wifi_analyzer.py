#!/usr/bin/env python3
"""
wifi_analyzer.py — WiFi Analyzer para auditoria e gestão de redes
Estilo gping + Wifiman: refresh rápido, UI rica em terminal, gráfico de sinal em tempo real.
...
Uso:
    ...
    python3 wifi_analyzer.py --debug -i wlo1
"""
...
def build_header_text() -> Text:
    ...
    for i, (w_line, a_line) in enumerate(zip(BANNER_WIFI, BANNER_ANALYZER)):
        ...
        t.append("\n")
    t.append("WiFi Analyzer  ·  Auditoria  ·  Gestão de Canais  ·  Anti-Interferência\n",
              style="dim")
    return t
...
SIGNAL_BARS = [
    (-50,  "[bold green]▂▄▆█[/bold green]  Excelente"),
    (-65,  "[bold green]▂▄▆[/bold green][dim]█[/dim]  Ótimo    "),
    (-75,  "[yellow]▂▄[/yellow][dim]▆█[/dim]  Bom      "),
    (-85,  "[orange3]▂[/orange3][dim]▄▆█[/dim]  Fraco    "),
    (-999, "[red]▂[/red][dim]▄▆█[/dim]  Muito fraco"),
]
...
def suggest_channel(networks: list, band: str) -> str:
    ...
    return f"CH {best} ({usage[best]} rede{'s' if usage[best]!=1 else ''})"
...
def _parse_nmcli_output(out: str) -> list:
    """Parseia a saída do nmcli -t e retorna lista de redes."""
    networks = []
...
def _trigger_rescan(iface: str):
    """Dispara um rescan assíncrono pelo NetworkManager."""
    global _rescan_proc, _rescan_iface, _last_rescan
    now = time.time()
    # Só dispara se o anterior já terminou e passou o intervalo mínimo
...
def scan_nmcli(iface: str) -> list:
    """
    Lê o cache do NM imediatamente (--rescan no) para resposta instantânea,
    enquanto dispara um rescan assíncrono em background para manter os dados frescos.
    """
...
def build_signal_graph(target: dict, history: deque, width: int, height: int = 28) -> Panel:
    """
    Gráfico de sinal estilo gping: linha fina no topo, sem preenchimento,
    escala dBm densa, ocupa toda a área disponível.
    """
    h = max(8, height)
    ...
    pad = w - 5
    lines.append(f"[dim]      {'mais antigo':<{pad//2}}{'agora':>{pad - pad//2}}[/dim]")
    ...
    title = (
        ...
        f"[{sc}]{sig} dBm[/{sc}]  "
        f"[dim]mín[/dim] [red]{mn}[/red]  "
        f"[dim]avg[/dim] [yellow]{avg}[/yellow]  "
        f"[dim]máx[/dim] [green]{mx}[/green] [dim]dBm[/dim]  "
        f"{signal_bar(sig)}"
    )
    return Panel(
        Text.from_markup("\n".join(lines)),
...
def build_focus_details(target: dict) -> Panel:
    ...
    lines = [
        ...
        f"[dim]Banda[/dim]       [{bc}]{target['band']}[/{bc}]",
        f"[dim]Canal[/dim]       [bold cyan]{target['channel']}[/bold cyan]",
        f"[dim]Frequência[/dim]  [cyan]{target.get('freq_mhz',0)} MHz[/cyan]",
        f"[dim]Largura[/dim]     {target.get('width',20)} MHz",
        f"[dim]Sinal[/dim]       [{sc}]{target['signal']} dBm[/{sc}]",
        f"[dim]Qualidade[/dim]   {signal_bar(target['signal'])}",
        f"[dim]Segurança[/dim]   [{sec_c}]{sec}[/{sec_c}]",
    ]
    return Panel(Text.from_markup("\n".join(lines)),
                 title="[bold]Detalhes[/bold]", border_style="bright_black", padding=(0,1))
...
def build_focus_peers(target: dict, networks: list) -> Panel:
    ...
    if not same:
        content = "[dim]Nenhuma rede no mesmo canal — sem interferência direta[/dim]"
    else:
        rows = []
    ...
    return Panel(Text.from_markup(content),
                 title=f"[bold yellow]Interferidores CH {target['channel']} / {target['band']}[/bold yellow]",
...
def build_table(networks: list, band_filter: str, sort_by: str, selected_idx: int) -> Table:
    ...
    table.add_column("Qualidade", width=22)
    table.add_column("Cong.",     width=6,  justify="center")
    table.add_column("Seg.",      width=10)
...
def build_channel_map(networks: list) -> Panel:
    ...
    content = Columns([col1, col2], equal=False, expand=True) if (col1 or col2) else Text("Sem dados")
    return Panel(content, title="[bold]Mapa de Canais[/bold]",
                 border_style="bright_black", padding=(0,1))

def build_suggestions(networks: list) -> Panel:
    s24 = suggest_channel(networks, "2.4G")
    s5  = suggest_channel(networks, "5G")
    txt = (f"[bold green]2.4 GHz[/bold green]  Canal sugerido: [bold white]{s24}[/bold white]\n"
           f"[bold cyan]5 GHz  [/bold cyan]  Canal sugerido: [bold white]{s5}[/bold white]")
    return Panel(txt, title="[bold]Sugestões Anti-Interferência[/bold]",
                 border_style="bright_black", padding=(0,1))
...
def build_stats(networks, iface, elapsed, mode, sel_idx, n_vis) -> Panel:
    ...
    best_s = (f"[bold white]{best['ssid']}[/bold white] [green]{best['signal']} dBm[/green]"
              if best else "—")
    mode_s = "[bold cyan]● FOCO[/bold cyan]" if mode=="focus" else "[dim]lista[/dim]"
...
HELP_LIST = (
    "[dim] [bold white]↑↓[/bold white]/[bold white]jk[/bold white] mover  "
    "[bold white]Enter[/bold white]/[bold white]f[/bold white] focar  "
    "[bold white]s[/bold white] signal  [bold white]c[/bold white] canal  "
    "[bold white]n[/bold white] ssid  [bold white]2[/bold white] 2.4G  "
    "[bold white]5[/bold white] 5G  [bold white]a[/bold white] todos  "
    "[bold white]m[/bold white] mapa  [bold white]q[/bold white] sair[/dim]"
)
HELP_FOCUS = (
    "[dim] [bold cyan]Esc[/bold cyan]/[bold cyan]b[/bold cyan] voltar  "
    "[bold white]↑↓[/bold white]/[bold white]jk[/bold white] trocar rede  "
    "[bold white]q[/bold white] sair[/dim]"
)
...
def main():
    ...
    if args.debug:
        ...
        for rescan in ("no","yes"):
            ...
            try:
                ...
                print("OUT:", r.stdout[:2000] or "(vazio)")
                print("ERR:", r.stderr[:300]  or "(vazio)")
    ...
    if not iface:
        ...
        if not ifaces:
            print("[ERRO] Nenhuma interface wireless encontrada."); sys.exit(1)
        iface = ifaces[0]
